...
from loader_s import SoundDataset
from prep_s import ProcessedDataset
...
metadata=pd.read_csv((r'SONYC\processed_SONYC.csv'))

metadata=metadata.iloc[:,1:]

sound_data = SoundDataset(metadata, r'SONYC\data/')
...
def train(args):
    ...
    if pretrained_name:
        model = get_mobilenet(width_mult=NAME_TO_WIDTH(pretrained_name), pretrained_name=pretrained_name,
                              head_type=args.head_type, se_dims=args.se_dims, num_classes=4)
    else:
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in pbar:
            ...
            # loss & accuracy
            loss = (torch.sum(samples_loss,dim=1)).mean()
            y_pre=((torch.sigmoid(y_hat))>=0.5).int()
            #print(y_pre)
            acc = metrics.accuracy_score(y.numpy(), y_pre.numpy())
        ...
        name = f"mn{str(args.model_width).replace('.', '')}_sonyc_epoch_{epoch}_mAP_{int(round(accuracy*100))}.pt"
